...
const ProductDetail = () => {
    ...
    const isLowStock = !isOutOfStock && totalInventory <= lowStockThreshold;
                    ...
                    <div className="grid lg:grid-cols-2 gap-12">
                        ...
                        >
                            ...
                            <div className="flex items-center gap-4 mb-6">
                                ...
                                <div className="flex items-center border border-border rounded-lg opacity-100 disabled:opacity-50">
                                    <button
                                    ...
                                    <button
                                        className="px-4 py-2 hover:bg-muted transition-colors disabled:opacity-50"
                                        onClick={() => setQuantity(Math.min(totalInventory, quantity + 1))}
                                        disabled={isOutOfStock || quantity >= totalInventory}
                                    >
                                        +
                                    </button>
                                </div>
                            ...
                            <Button
                                size="lg"
                                className="w-full bg-secondary hover:bg-secondary/90 disabled:bg-muted disabled:text-muted-foreground"
                                onClick={handleAddToCart}
                                disabled={isOutOfStock}
                            >
                                <ShoppingCart className="w-5 h-5 mr-2" />
                                {isOutOfStock ? 'Out of Stock' : `Add to Cart - ${(parseFloat(price) * quantity).toFixed(2)}`}
                            </Button>
                            ...
                            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2 p-2 rounded-lg bg-black/50 backdrop-blur-sm">
                                {images.map((image, index) => (
                                    <button
                                        ...
                                        className={`w-14 h-14 rounded-md overflow-hidden border-2 transition-all ${selectedImageIndex === index
