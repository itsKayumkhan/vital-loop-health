...
export const CartDrawer = () => {
    ...
    const handleCheckout = async () => {
        if (!user) {
            ...
            setIsOpen(false);
            navigate("/auth?redirect=supplements");
            return;
        }

        try {
            const result = await submitOrder(user.id);
            if (result?.url) {
                // Redirect to Stripe
                window.location.href = result.url;
            } else if (result?.orderId) {
                // Fallback or non-Stripe success
                setIsOpen(false);
                navigate("/orders");
            }
        } catch (error) {
                    ...
                    {items.length === 0 ? (
                        <div className="flex-1 flex items-center justify-center">
                            <div className="text-center">
                                ...
                                <p className="text-sm text-muted-foreground mt-2">Add some supplements to get started</p>
                            </div>
                        </div>
                    ) : (
                        <>
